...
def mpcem(coords, load, is_support, is_origin_node, edge_index, is_trail_edge, length, force_sign, force,
          max_iter=100, tolerance=1e-8, verbose=False):
    ...
    for i in range(max_iter):
        ...
        valid_nodes = ~torch.isnan(coords).any(dim=1)  # only consider edges with a coordinate (estimate) for both nodes
        valid_edges = valid_nodes[edge_index[0]] & valid_nodes[edge_index[1]]
        residual_force = residual_force_update(coords, force[valid_edges], edge_index[:, valid_edges], load)
        ...
        trail_force_magnitude = torch.norm(trail_force, dim=1)
        force[is_trail_edge] = force_sign[is_trail_edge] * trail_force_magnitude

        # Update coordinates
        unit_trail_force = trail_force / trail_force_magnitude.clamp(min=1e-8).unsqueeze(1)
        coords[trail_dst] = coords[trail_src] + unit_trail_force * length[is_trail_edge].unsqueeze(1) * -force_sign[is_trail_edge].unsqueeze(1)
...
def cem(coords, load, is_support, is_origin_node, edge_index, is_trail_edge, length, force_sign, force, sequence,
          max_iter=100, tolerance=1e-8, verbose=False):
    ...
    for i in range(max_iter):
        ...
        for k in range(max_k):
            # Calculate outgoing trail force
            k_mask = sequence == k
            valid_nodes = ~torch.isnan(coords).any(dim=1)  # only consider edges with a coordinate (estimate) for the source node and current sequence for the destination node
            ...
            trail_force_magnitude = torch.norm(trail_force, dim=1)
            force[current_trail_edge] = force_sign[current_trail_edge] * trail_force_magnitude

            # Update coordinates
            current_trail_src, current_trail_dst = edge_index[:, current_trail_edge]
            unit_trail_force = trail_force / trail_force_magnitude.clamp(min=1e-8).unsqueeze(1)
            coords[current_trail_dst] = coords[current_trail_src] + unit_trail_force * length[current_trail_edge].unsqueeze(1) * -force_sign[current_trail_edge].unsqueeze(1)
