...
def tna(coords, is_support, load, edge_index, q_target, verbose=False):  # only works for directed graphs
    force_density = least_squares_tna(coords, is_support, edge_index, q_target, verbose)
    coords, force = fdm(coords, load, is_support, edge_index, force_density, directed=True, use_batching=False, solve_only_z=True)
    return coords, force, force_density


def least_squares_tna(coords, is_support, edge_index, q_target, verbose=False):  # only works for directed graphs
    """
    ...
    if verbose:
        print(f"Least Squares TNA number of degrees of freedom: {basis_vectors.shape[1]}")
