...
class CableStayedBridge:
    ...
    def generate_graph(self):
        ...
        # Deck
        deck_origin_coords = [torch.tensor([-0.5*self.deck_trail_length, 0.0, 0.0]),
        ...
        for i in range(2):
            ...
            for j in range(1, self.n_deck_trail_edges):
                support_condition = torch.tensor([True, True, True]) if j == self.n_deck_trail_edges - 1 else torch.tensor([False, False, False])
                self.graph.add_node(
        ...
        for i in range(self.n_towers):
            ...
            tower_origin_coords = torch.tensor([tower_x[i], tower_offset, self.tower_height])
            self.graph.add_node(
            ...
            for j in range(1, self.n_cables + 1):
                support_condition = torch.tensor([True, True, True]) if j == self.n_cables else torch.tensor([False, False, False])
                length = self.tower_height*1.3 if j == self.n_cables else self.tower_trail_length
                self.graph.add_node(
        ...
        # Cables
        deck_nodes = ([f"deck_trail_0_node_{i}" for i in reversed(range(self.n_deck_trail_edges))] + 
                      [f"deck_trail_1_node_{i}" for i in range(self.n_deck_trail_edges)])
        cable_nodes = [
            f"tower_trail_{i}_node_{j}"
            for i in range(self.n_towers)
            for reverse in [False, True]
            for j in (reversed(range(self.n_cables)) if reverse else range(self.n_cables))
        ]
