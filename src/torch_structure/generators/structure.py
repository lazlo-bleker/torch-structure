...
class Structure:
    ...
    def _set_node(self, node_idx, coords, load, sem_type, external_force=None, support=False, origin=False):
    ...
    def _force_direction(self, start_node_idx, end_node_idx):
        ...
        direction = self.nodes[end_node_idx]['coordinates'] - self.nodes[start_node_idx]['coordinates']
    ...
    def _resultant(self, node_idx, load_factor=1.0, ignore_sem_type=None):
        ...
        for edge, attributes in self.edges.items():
            if node_idx in edge:
                if ignore_sem_type is not None and self.edges[edge]['semantic_type'] == ignore_sem_type:
                    continue
