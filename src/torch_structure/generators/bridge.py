...
class Bridge(Structure):
    ...
    def __init__(self, **kwargs):
        ...
        for param, value in kwargs.items():
            if param in self.param_dtypes and not np.issubdtype(type(value), self.param_dtypes[param]):
                raise TypeError(f"Expected type {self.param_dtypes[param]} for parameter '{param}', got {type(value)}")
    ...
    def sample_input(span=None):
        ...
        params = {
            ...
            'n_bays': np.random.randint(math.ceil(0.5*span/7), math.floor(0.5*span/3) + 1),  # max, min bay_size = 7, 3
            'midspan_height': (
                span * cable_force_sign * np.random.uniform(-0.3, -0.1)
                if braced else span * cable_force_sign * np.random.uniform(-0.2, 0.05)
            ),
            ...
            'deck_rise': 0.0 if braced else np.random.choice([0.0, np.random.uniform(0.02, 0.1)], p=[0.7, 0.3]),
            ...
            'inter_cable_force': 0.0 if braced else np.random.choice([0, np.random.uniform(-1.0, 1.0)])
        }
        return params
    ...
    def formfind(self, threshold=5e-4, update_straight_trail_force=True):
        ...
        self._set_node(0, np.array([0.0, 0.5*self.deck_width, 0.0]), 0.5*self.load,
                       external_force=np.array([-self.deck_force, 0.0, 0.0]), sem_type='deck', origin=True)
        self._set_node(1, np.array([0.0, -0.5*self.deck_width, 0.0]), 0.5*self.load,
                       external_force=np.array([-self.deck_force, 0.0, 0.0]), sem_type='deck', origin=True)
        ...
        if self.n_cables == 1:
            self._set_node(2, np.array([0.0, 0.0, self.midspan_height]), np.zeros(3),
                           external_force=np.copy(cable_force_vector), sem_type='cable', origin=True)
        else:
            self._set_node(2, np.array([0.0, 0.5*self.cable_distance, self.midspan_height]), np.zeros(3),
                           external_force=np.copy(cable_force_vector), sem_type='cable', origin=True)
            self._set_node(3, np.array([0.0, -0.5*self.cable_distance, self.midspan_height]), np.zeros(3),
                           external_force=np.copy(cable_force_vector), sem_type='cable', origin=True)
        ...
        for i in range(self.n_bays):
            ...
            left_hangar_force = self._hangar_force(prev_left_deck_node_idx, prev_left_cable_node_idx, z_offset=z_offset)
            self._set_edge((prev_left_deck_node_idx, prev_left_cable_node_idx),
                           left_hangar_force, 'deviation', 'hangar')
            cable_node_idx = prev_left_cable_node_idx if self.n_cables == 1 else prev_right_cable_node_idx
            right_hangar_force = self._hangar_force(prev_right_deck_node_idx, cable_node_idx, z_offset=z_offset)
            self._set_edge((prev_right_deck_node_idx, cable_node_idx),
                           right_hangar_force, 'deviation', 'hangar')

            # Set inter-deck edge
            inter_deck_force = self._inter_deck_force(prev_left_deck_node_idx, prev_right_deck_node_idx)
            self._set_edge((prev_left_deck_node_idx, prev_right_deck_node_idx),
                           inter_deck_force, 'deviation', 'inter-deck')


            # Determine next deck nodes and edges
            next_left_deck_coords, left_deck_force = self._next_node(prev_left_deck_node_idx, self.bay_size)
            self._set_node(next_left_deck_node_idx, next_left_deck_coords, self.load, sem_type='deck')
            self._set_edge((prev_left_deck_node_idx, next_left_deck_node_idx), left_deck_force, 'trail', 'deck')
            next_right_deck_coords, right_deck_force = self._next_node(prev_right_deck_node_idx, self.bay_size)
            self._set_node(next_right_deck_node_idx, next_right_deck_coords, self.load, sem_type='deck')
            self._set_edge((prev_right_deck_node_idx, next_right_deck_node_idx), right_deck_force, 'trail', 'deck')

            # Determine brace forces
            if self.brace_force != 0.0:
                brace_factor = self.brace_force / (-self.n_cables + 3)
                left_brace_force = self._brace_force(prev_left_cable_node_idx, next_left_deck_node_idx,
                                                     factor=brace_factor)
                right_brace_force = self._brace_force(cable_node_idx, next_right_deck_node_idx, factor=brace_factor)
                self._set_edge((prev_left_cable_node_idx, next_left_deck_node_idx),
                               left_brace_force, 'deviation', 'brace')
                self._set_edge((cable_node_idx, next_right_deck_node_idx), right_brace_force, 'deviation', 'brace')

            # Set inter-cable edges
            if self.n_cables == 2 and self.inter_cable_force != 0.0:
                self._set_edge((prev_left_cable_node_idx, prev_right_cable_node_idx), self.inter_cable_force,
                               'deviation', 'inter-cable')

            # Determine next cable node and edge
            next_left_cable_coords, cable_force = self._next_node(prev_left_cable_node_idx, self.bay_size)
            self._set_node(next_left_cable_node_idx, next_left_cable_coords, np.zeros(3), sem_type='cable')
            self._set_edge((prev_left_cable_node_idx, next_left_cable_node_idx), cable_force, 'trail', 'cable')
            if self.n_cables == 2:
                next_right_cable_coords, cable_force = self._next_node(prev_right_cable_node_idx, self.bay_size)
                self._set_node(next_right_cable_node_idx, next_right_cable_coords, np.zeros(3), sem_type='cable')
                self._set_edge((prev_right_cable_node_idx, next_right_cable_node_idx), cable_force, 'trail', 'cable')

        # Add reaction force as external force
        support_nodes = [next_left_deck_node_idx, next_right_deck_node_idx, next_left_cable_node_idx]
        if self.n_cables == 2:
        ...
        if update_straight_trail_force:
            ...
            deck_straight = (1 - self._force_direction(prev_left_deck_node_idx, next_left_deck_node_idx)[0] < threshold)
            if deck_straight and abs(self.nodes[next_left_deck_node_idx]['external_force'][0]) > threshold:
                deck_force_update = -self.nodes[next_left_deck_node_idx]['external_force'][0] - 1e-4
                self.params['deck_force'] += deck_force_update
                self.nodes[next_left_deck_node_idx]['external_force'][0] += deck_force_update
                self.nodes[0]['external_force'][0] -= deck_force_update
                self.nodes[next_right_deck_node_idx]['external_force'][0] += deck_force_update
            ...
            cable_straight = (1 - self._force_direction(prev_left_cable_node_idx,next_left_cable_node_idx)[0] < threshold)
            if cable_straight and abs(self.nodes[next_left_cable_node_idx]['external_force'][0]) > threshold:
                cable_force_update = -self.nodes[next_left_cable_node_idx]['external_force'][0] - 1e-5
                self.params['cable_force'] += cable_force_update
                self.nodes[next_left_cable_node_idx]['external_force'][0] += cable_force_update
                self.nodes[2]['external_force'][0] -= cable_force_update
                if self.n_cables == 2:
                    self.nodes[next_right_cable_node_idx]['external_force'][0] += cable_force_update
    ...
    def mirror_structure(self):
        ...
        mirrored_nodes = {}
        nodes_to_skip = {0: 1, 1: 0, 2: 2} if self.n_cables == 1 else {0: 1, 1: 0, 2: 3, 3: 2}
        ...
        for (start_idx, end_idx), edge_info in self.edges.items():
            mirrored_start_idx = nodes_to_skip[start_idx] if start_idx in nodes_to_skip else start_idx + new_idx_offset
            mirrored_end_idx = nodes_to_skip[end_idx] if end_idx in nodes_to_skip else end_idx + new_idx_offset
    ...
    def array_output(self):
        ...
        self.array_dict['trail_edge'] = np.array([edge['cem_type'] == 'trail' for edge in self.edges.values()])
        self.array_dict['deviation_edge'] = np.array([edge['cem_type'] == 'deviation' for edge in self.edges.values()])
        self.array_dict['deck_edge'] = np.array([edge['semantic_type'] == 'deck' for edge in self.edges.values()])
        self.array_dict['cable_edge'] = np.array([edge['semantic_type'] == 'cable' for edge in self.edges.values()])
        self.array_dict['hangar_edge'] = np.array([edge['semantic_type'] == 'hangar' for edge in self.edges.values()])
        self.array_dict['force_edge'] = np.array([edge['force'] for edge in self.edges.values()])
        self.array_dict['length_edge'] = np.array([edge['length'] for edge in self.edges.values()])

        # Save node attributes
        self.array_dict['coordinates_node'] = np.array([node['coordinates'] for node in self.nodes.values()])
        self.array_dict['load_node'] = np.array([node['load'] for node in self.nodes.values()])
        self.array_dict['support_node'] = np.array([node['support'] for node in self.nodes.values()])
        self.array_dict['support_node_indices'] = np.where(self.array_dict['support_node'])
        self.array_dict['origin_node'] = np.array([node['origin'] for node in self.nodes.values()])
        self.array_dict['origin_node_indices'] = np.where(self.array_dict['origin_node'])
        self.array_dict['deck_node'] = np.array([node['semantic_type'] == 'deck' for node in self.nodes.values()])
        self.array_dict['deck_node_indices'] = np.where(self.array_dict['deck_node'])
        self.array_dict['cable_node'] = np.array([node['semantic_type'] == 'cable' for node in self.nodes.values()])
    ...
    def pyg_data(self):
        ...
        # Node features
        coordinates_node = torch.tensor(self.array_dict['coordinates_node'], dtype=torch.float)
        load_node = torch.tensor(self.array_dict['load_node'], dtype=torch.float)
        support_node = torch.tensor(self.array_dict['support_node'], dtype=torch.float).unsqueeze(1)
        meta_data['node_features'] = {
            ...
            6: {'description': 'support condition (0 = free, 1 = pinned)', 'type': 'categorical'}}
        node_features = torch.cat([coordinates_node, load_node, support_node], dim=1)

        # Edge features
        force_edge = torch.tensor(self.array_dict['force_edge'], dtype=torch.float).unsqueeze(1)
        length_edge = torch.tensor(self.array_dict['length_edge'], dtype=torch.float).unsqueeze(1)
        ...
        # Edge index
        edge_index = torch.tensor(self.array_dict['edge_index'], dtype=torch.long).t().contiguous()

        undirected_edge_index, undirected_edge_features = torch_geometric.utils.to_undirected(edge_index, edge_attr=edge_features)
    ...
    def plot(self, ax=None, plot_threshold=1e-4, plot_deck=True, path=None):
        ...
        for edge in self.edges:
            ...
            x_coords = (self.nodes[edge[0]]['coordinates'][0], self.nodes[edge[1]]['coordinates'][0])
            y_coords = (self.nodes[edge[0]]['coordinates'][1], self.nodes[edge[1]]['coordinates'][1])
            z_coords = (self.nodes[edge[0]]['coordinates'][2], self.nodes[edge[1]]['coordinates'][2])
            ax.plot(x_coords, y_coords, z_coords, color=color)

        if plot_deck:
            self.array_output()
            coords = self.array_dict['coordinates_node'][self.array_dict['deck_node']]
            coords = np.concatenate((coords[0:(self.n_bays + 1)*2:2], coords[(self.n_bays + 1)*2 - 1:0:-2],
                                     coords[(self.n_bays + 2)*2::2], coords[-1:(self.n_bays + 1)*2 + 1:-2]))
        ...
        ax.set_title(f'Span = {self.span:.1f} | LP = {load_path:.1f}', color=self.title_color)
        if path:
