...
class NetwokrkArchBridge:
    ...
    def generate_graph(self):
        ...
        # Deck
        deck_origin_coords = [torch.tensor([-0.5*self.deck_trail_length, -0.5*self.deck_width, 0.0]),
        ...
        for i in range(len(deck_origin_coords)):
            ...
            for j in range(1, self.n_trail_edges + 1):
                support_condition = torch.tensor([True, True, True]) if j == self.n_trail_edges else torch.tensor([False, False, False])
                load = torch.zeros(3, dtype=torch.float) if j == self.n_trail_edges else self.deck_load
                x_target = j*self.deck_trail_length*torch.sign(deck_origin_coords[i][0])
                self.graph.add_node(
                    ...
                    target_coords=deck_origin_coords[i] + torch.tensor([x_target, 0.0, 0.0]),
                    bridge_side=i % 2,
                )
                self.graph.add_edge(
        ...
        # Arches
        arch_origin_coords = [torch.tensor([-0.5*self.arch_trail_length, -0.5*self.arch_width, self.height]),
                              torch.tensor([0.5*self.arch_trail_length, -0.5*self.arch_width, self.height]),
                              torch.tensor([-0.5*self.arch_trail_length, 0.5*self.arch_width, self.height]),
                              torch.tensor([0.5*self.arch_trail_length, 0.5*self.arch_width, self.height])]
        for i in range(len(arch_origin_coords)):
            ...
            for j in range(1, self.n_trail_edges + 1):
                support_condition = torch.tensor([True, True, True]) if j == self.n_trail_edges else torch.tensor([False, False, False])
                x_target = j*self.deck_trail_length*torch.sign(deck_origin_coords[i][0])
                target_coords = deck_origin_coords[i] + torch.tensor([x_target, 0.0, 0.0]) if j == self.n_trail_edges else torch.full((3,), torch.nan)
                self.graph.add_node(
        ...
        # Prepare node lists
        deck_nodes_0 = ([f"deck_trail_0_node_{i}" for i in reversed(range(self.n_trail_edges + 1))] + 
                    [f"deck_trail_1_node_{i}" for i in range(self.n_trail_edges + 1)])
        deck_nodes_1 = ([f"deck_trail_2_node_{i}" for i in reversed(range(self.n_trail_edges + 1))] +
                    [f"deck_trail_3_node_{i}" for i in range(self.n_trail_edges + 1)])
        arch_nodes_0 = ([f"arch_trail_0_node_{i}" for i in reversed(range(self.n_trail_edges + 1))] +
                    [f"arch_trail_1_node_{i}" for i in range(self.n_trail_edges + 1)])
        arch_nodes_1 = ([f"arch_trail_2_node_{i}" for i in reversed(range(self.n_trail_edges + 1))] +
                    [f"arch_trail_3_node_{i}" for i in range(self.n_trail_edges + 1)])

        # Cables
        if self.kinematic_cable_arrangement:
            for deck_nodes, arch_nodes in zip([deck_nodes_0, deck_nodes_1], [arch_nodes_0, arch_nodes_1]):
                ...
                for i, (arch_node, deck_node) in enumerate(zip(arch_nodes_cables_1, deck_nodes_cables_1)):
                    self.graph.add_edge(
                        arch_node,
                        deck_node,
                        is_trail_edge=torch.tensor(False),
                ...
                for i, (arch_node, deck_node) in enumerate(reversed(list(zip(arch_nodes_cables_2, deck_nodes_cables_2)))):
                    self.graph.add_edge(
                        arch_node,
                        deck_node,
                        is_trail_edge=torch.tensor(False),
            ...
            self.cable_optim_groups = np.arange(self.n_optim_groups, self.n_optim_groups + i + 1)
            self.n_optim_groups += i + 1
            
        else:   
            for deck_nodes, arch_nodes in zip([deck_nodes_0, deck_nodes_1], [arch_nodes_0, arch_nodes_1]):
                for i, (arch_node, deck_node) in enumerate(zip(arch_nodes[self.cable_offset + 1:-1], deck_nodes[1:-self.cable_offset + 1])):
                    self.graph.add_edge(
                        arch_node,
                        deck_node,
                        is_trail_edge=torch.tensor(False),
                ...
                for i, (arch_node, deck_node) in enumerate(reversed(list(zip(arch_nodes[1:-self.cable_offset + 1], deck_nodes[self.cable_offset + 1:-1])))):
                    self.graph.add_edge(
                        arch_node,
                        deck_node,
                        is_trail_edge=torch.tensor(False),
