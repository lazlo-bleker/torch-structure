...
class StructData(TSMixin, pyg.data.Data):
    ...
    def add_nodes(self, names = None, symmetry: str = None, **kwargs):
        ...
        Args:
            names (list[str], optional): node name for each new node. Encoded and
                ...
                node's name is copied verbatim to all of its symmetry copies.
            symmetry (str, optional): name of a registered symmetry to apply. If given,
                each provided node (seed) is copied according to the symmetry,
                together with all of its attributes. Attributes registered as
                ``transform_attrs`` are geometrically transformed, every other
                attribute is copied verbatim. Each seed together with its copies gets
                a new orbit id attribute (global), counting up from the highest existing one. 
                Additionally, they get an orbit position attribute (local) counting up from 0, 
                in the order they are generated by the symmetry. 
        ...
        Raises:
            ValueError: if no keyword arguments are provided. If you want to add nodes
                without any attributes, use ``add_n_empty_nodes()``.
            ValueError: if ``symmetry`` is not a registered symmetry.
            ValueError: if the attribute tensors are empty or have different lengths.
            ValueError: if ``names`` does not have the same length as the attribute tensors.
        ...
        if symmetry is not None:
            if symmetry not in self.metadata.get("name_to_symmetry", {}):
    ...
    def _expand_edges_symmetrically(self, edge_indices, kwargs):
        ...
        for group_id in torch.unique(sym_group_ids).tolist():
            ...
            for attr, value in kwargs.items():
                kwargs_expanded[attr].append(value[group_edge_idx][seed_edge])
    ...
    def add_edges(self, edge_indices, consider_symmetry: bool = True, **kwargs):
        ...
        Raises:
            ...
            ValueError: if an edge would be added more than once, i.e. it is given twice
                or, with ``consider_symmetry=True``, the given edges are symmetry copies of
                each other.
        """
        ...
        def undirected_keys(edges):
            return torch.minimum(edges[0], edges[1]) * self.num_nodes + torch.maximum(edges[0], edges[1])

        unique_keys, key_counts = torch.unique(undirected_keys(edge_indices), return_counts=True)

        added_twice = unique_keys[key_counts > 1]
        if added_twice.numel() > 0:
            raise ValueError(
                f"Edge(s) {[divmod(key, self.num_nodes) for key in added_twice.tolist()]} would be added more than "
                "once: they are given twice, or they are symmetry copies of each other (with "
                "consider_symmetry=True, give only one edge per edge family)."
            )

        existing = unique_keys[torch.isin(unique_keys, undirected_keys(self.edge_index))]
        if existing.numel() > 0:
            raise ValueError(f"Edge(s) {[divmod(key, self.num_nodes) for key in existing.tolist()]} already exist.")
    ...
    def _build_orbit_lookup(self):
        orbit_id = self.orbit_id.view(-1)
        orbit_position = self.orbit_position.view(-1)
        # nodes outside any symmetry have orbit id and position -1 and must not be registered
        tracked = torch.where(orbit_id != -1)[0]

        num_orbits = int(orbit_id.max().item()) + 1 if orbit_id.numel() > 0 else 0
        num_positions = int(orbit_position.max().item()) + 1 if orbit_position.numel() > 0 else 0

        lookup = torch.full((num_orbits, num_positions), -1, dtype=torch.long)
        lookup[orbit_id[tracked], orbit_position[tracked]] = tracked
        return lookup
    ...
    def add_edges_by_orbit(self, src_orbit_ids, dest_orbit_ids, src_orbit_positions, dest_orbit_positions, consider_symmetry: bool = True, **kwargs):
        ...
        Args:
            src_orbit_ids (list[int]): orbit id for each new edge's source. 
            dest_orbit_ids (list[int]): orbit id for each new edge's destination.
    ...
    @staticmethod
    def _check_vector(name, vector, nonzero=False):
        # the symmetry matrices use the default float dtype, so other dtypes would fail later
        if not isinstance(vector, torch.Tensor):
            raise ValueError(f"{name} must be a torch.Tensor of shape [3], got {type(vector).__name__}.")

        if vector.shape != (3,) or vector.dtype != torch.get_default_dtype():
            raise ValueError(
                f"{name} must have shape [3] and dtype {torch.get_default_dtype()}, "
                f"got shape {list(vector.shape)} and dtype {vector.dtype}."
            )

        if nonzero and not torch.any(vector != 0):
    ...
    def create_rotational_symmetry(self, n, origin = torch.tensor([0.0, 0.0, 0.0]), rotation_axis = torch.tensor([0.0, 0.0, 1.0])) -> dict[str, torch.Tensor]:
        ...
        Raises:
            ...
            ValueError: if ``origin`` or ``rotation_axis`` is not a tensor of shape [3] with the
                default float dtype.
            ValueError: if ``rotation_axis`` is the zero vector.
        """

        if n < 1:
            raise ValueError(f"n must be at least 1, got {n}.")

        self._check_vector("origin", origin)
        self._check_vector("rotation_axis", rotation_axis, nonzero=True)
    ...
    def create_mirror_symmetry(self, origin = torch.tensor([0.0, 0.0, 0.0]), normal = torch.tensor([1.0, 0.0, 0.0])) -> dict[str, torch.Tensor]:
        """
        Builds a mirror symmetry with respect to the plane through ``origin`` with normal
        ``normal``.
        ...
        self._check_vector("origin", origin)
        self._check_vector("normal", normal, nonzero=True)
    ...
    def combine_symmetry(self, symmetry_a, symmetry_b) -> dict[str, torch.Tensor]:
        """
        Nests ``symmetry_a`` inside ``symmetry_b``, producing every combination of the
        two.

        Args:
            symmetry_a (dict): the inner symmetry, as returned by
                ``create_rotational_symmetry``, ``create_mirror_symmetry``, or a previous
                ``combine_symmetry`` call.
    ...
    def set_node_attr(self, attr: str, mask: torch.Tensor, value: torch.Tensor, consider_symmetry: bool = True):
        ...
        Args:
            ...
            consider_symmetry (bool): if ``True`` (default) and the graph has a registered
                symmetry, the update propagates to all symmetry copies of each selected
                node (the rest of its orbit), not just the nodes selected by ``mask``.
                Attributes registered as ``transform_attrs`` are geometrically transformed
                per copy, attributes registered as ``copy_attrs`` are copied verbatim.
                If ``False``, only the selected nodes are updated.
        ...
        Raises:
            ...
            ValueError: if ``consider_symmetry`` is ``True`` and two selected nodes belong to
                the same orbit.
            ValueError: if ``consider_symmetry`` is ``True`` and a selected node's orbit is
                no longer intact, e.g. because some of its nodes were merged or removed.
            ValueError: if ``consider_symmetry`` is ``True`` and a selected node's symmetry
                group has ``attr`` classified as neither a transform nor a copy attribute.
        """
        ...
        if torch.any(torch.bincount(node_orbit_ids[tracked]) > 1):
            raise ValueError("Multiple selected nodes belong to the same orbit. Select only one node per orbit.")

        tracked_nodes = nodes[tracked]
        tracked_values = value[tracked]
        tracked_group_ids = symmetry_id[tracked_nodes]

        # compute the new values of every orbit before writing anything, so a failed check leaves
        ...
        for group_id in torch.unique(tracked_group_ids).tolist():
            ...
            orbit_updates.append((members.reshape(-1), new_values))

        current[nodes] = value
        for member_rows, new_values in orbit_updates:
    ...
    def set_node_attr_by_name(self, attr: str, names: list, value: torch.Tensor, consider_symmetry: bool = True):
        """
        Resolves node names to indices, then calls ``set_node_attr``. Each name selects
        every node that carries it. Symmetry copies created by ``add_nodes`` share
        their seed's name, so a name selects a whole orbit in this case.
        ...
        Args:
            ...
            consider_symmetry (bool): if ``True`` (default) and the graph has a registered
                symmetry, one matching node per orbit is kept for a copy attribute, and
                ``value`` is propagated to the rest of its orbit, see ``set_node_attr``.
                Matching nodes outside any symmetry are all kept. If ``False``, every
                matching node is set to the given value.

        Raises:
            ValueError: if ``names`` contains duplicates.
            ValueError: if ``value`` does not have one row per entry in ``names``.
            ValueError: if a name in ``names`` does not match any node.
            ValueError: if ``consider_symmetry`` is ``True``, ``attr`` is a transform attribute
                and a name matches several nodes of the same orbit.
            ValueError: any error raised by ``set_node_attr`` for the resolved nodes.
        """
        if len(set(names)) != len(names):
            raise ValueError(f"names must not contain duplicates, got {names}.")

        if value.shape[0] != len(names):
            raise ValueError(f"value must have one row per name ({len(names)}), but has {value.shape[0]}.")

        indices, counts = [], []
        for name in names:
            nodes = self.get_node_index_from_name(name)
            if nodes.numel() == 0:
                raise ValueError(f"No node found with name '{name}'.")

            if consider_symmetry and hasattr(self, "symmetry_id"):
                orbit_id = self.orbit_id.view(-1)[nodes]
                repeated = torch.zeros_like(orbit_id, dtype=torch.bool)
                repeated[1:] = (orbit_id[1:] == orbit_id[:-1]) & (orbit_id[1:] != -1)

                repeated_groups = self.symmetry_id.view(-1)[nodes[repeated]].unique().tolist()
                if any(attr in self.metadata["symmetry_transform_attrs"][group] for group in repeated_groups):
                    raise ValueError(
                        f"Name '{name}' matches several nodes of the same orbit, but '{attr}' is a "
                        "transform attribute. Select one node per orbit, e.g. via set_node_attr_by_orbit()."
                    )

                nodes = nodes[~repeated]

            indices.extend(nodes.tolist())
            counts.append(nodes.numel())

        indices = torch.tensor(indices, dtype=torch.long)
        value = value.repeat_interleave(torch.tensor(counts, dtype=torch.long), dim=0)
    ...
    def set_node_attr_by_orbit(self, attr: str, orbit_ids: list, orbit_positions: list, value: torch.Tensor, consider_symmetry: bool = True):
        ...
        Args:
            attr (str): name of the registered node attribute to set.
            orbit_ids (list[int]): orbit id for each row of ``value`` as given by ``add_nodes``.
            orbit_positions (list[int]): position in the orbit for each row of ``value`` as given by ``add_nodes``.
    ...
    def set_edge_attr_by_orbit(self, attr: str, src_orbit_ids: list, dest_orbit_ids: list, src_orbit_positions: list, dest_orbit_positions: list, value: torch.Tensor, consider_symmetry: bool = True):
        ...
        Args:
            ...
            src_orbit_ids (list[int]): orbit id for each edge's source as given by ``add_nodes``.
            dest_orbit_ids (list[int]): orbit id for each edge's destination as given by ``add_nodes``.
            src_orbit_positions (list[int]): position in the orbit for each edge's source as given by ``add_nodes``.
    ...
    def set_edge_attr(
        ...
        Raises:
            ...
            ValueError: if ``mask`` does not have ``num_edges`` entries.
            ValueError: if ``value`` does not have one row per ``True`` entry in ``mask``.
            ...
            ValueError: if ``consider_symmetry`` is ``True`` and ``attr`` is not registered
                as a copy attribute for a selected edge's symmetry.
            ValueError: if ``consider_symmetry`` is ``True`` and a symmetry copy of a selected edge
                does not exist.
            ValueError: if ``consider_symmetry`` is ``True`` and two selected edges are
                symmetry copies of each other but were given different values.
        """
        ...
        if mask.shape[0] != num_edges:
            raise ValueError(f"mask must have {num_edges} entries, but has {mask.shape[0]}.")
        ...
        src, dst, tracked_value, src_group, dst_group = (
        ...
        # compute the copies of every group before writing anything, so a failed check leaves the
        # attribute unchanged
        sibling_updates = []
        for group_id in torch.unique(src_group).tolist():
            if attr not in self.metadata["symmetry_copy_attrs"][group_id]:
                raise ValueError(
                    ...
                    f"group {group_id}. Register it via add_symmetry(copy_attrs=...)."
                )
            ...
            if not torch.all(sorted_keys[pos] == keys):
                raise ValueError("No edge found for a symmetry copy of a selected edge.")
            sib_edges = sort_idx[pos]

            
            sib_forward = torch.where(directed[sib_edges], sib_edges, self.reciprocal_edge.view(-1)[sib_edges])
            order = torch.argsort(sib_forward)
            same_edge = sib_forward[order][1:] == sib_forward[order][:-1]
            sorted_value = sib_value[order].reshape(order.shape[0], -1)
            different_value = torch.any(sorted_value[1:] != sorted_value[:-1], dim=1)
            if torch.any(same_edge & different_value):
                raise ValueError(
                    "Multiple selected edges are symmetry copies of each other but were given "
                    "different values. Select one edge per edge family, or give them the same value."
                )

            sibling_updates.append((sib_edges, sib_value))

        self._modify_edge_attr_and_reciprocal(attr, edges, value)
        for sib_edges, sib_value in sibling_updates:
            self._modify_edge_attr_and_reciprocal(attr, sib_edges, sib_value)
    ...
    def add_symmetry(self, symmetries: dict, transform_attrs: list = None, copy_attrs: list = None):
        ...
        All symmetries passed in one call share the same ``transform_attrs`` and
        ``copy_attrs``; register symmetries in separate calls to give them different
        attributes.
        ...
        Args:
            ...
            copy_attrs (list[str], optional): node and edge attributes that should be copied
                verbatim to every copy of a node or edge, without any transform.

        Raises:
            ValueError: if ``transform_attrs``/``copy_attrs`` contains symmetry bookkeeping
                (``orbit_id``, ``orbit_position``, ``symmetry_id``).
        ...
        if invalid_transform_attrs:
            raise ValueError(
                ...
                "registered as a transform attribute; edge attributes can only be copy attributes (copy_attrs=...)."
            )
        ...
        for name, symmetry in symmetries.items():
            ...
            self.metadata["symmetry_copy_attrs"][group_id] = list(copy_attrs)
    ...
    def add_triangular_grid(self, n, node_attrs: dict = {}, edge_attrs: dict = {}):
        ...
        Args:
            n (int): number of nodes along each side of the triangle. Must be at least 2.
            node_attrs (dict[str, torch.Tensor | callable]): mapping of registered
                node attribute names to either a tensor of shape ``[N, *]`` or
                ``[1, *]`` (broadcast over all nodes), or a callable whose
                parameter names are resolved from the node defaults listed above as well as any
    ...
    def add_polar_grid(self, n_sectors, n_rings, node_attrs: dict = {}, edge_attrs: dict = {}):
        ...
        Raises:
            ValueError: if ``n_rings`` is less than 2.
            ValueError: if ``n_sectors`` is less than 3.
        """


        if n_rings <= 1:
            raise ValueError(f"n_rings must be greater than 1, got {n_rings}.")
        if n_sectors <= 2:
            raise ValueError(f"n_sectors must be greater than 2, got {n_sectors}.")
