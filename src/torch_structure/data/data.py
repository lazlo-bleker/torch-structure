...
class Data:
    _internal_attrs = {
        "data", "default_attrs", "node_name_to_index",
        "node_attr_list", "edge_attr_list", "graph_attr_list", "_num_nodes"
    }

    def __init__(self, edge_index=torch.empty((2, 0), dtype=torch.long), node_attrs={}, edge_attrs={},
                 graph_attrs={}, default_attrs={}):
        directed_mask = torch.empty((0, 1), dtype=torch.bool)
        reciprocal_edge = torch.empty((0, 1), dtype=torch.long)
        self.data = pyg.data.Data(edge_index=edge_index, directed_mask=directed_mask, reciprocal_edge=reciprocal_edge, 
    ...
    def is_support(self):
        ...
        elif 'support_condition' in self.node_attr_list:  # If is_support is missing, fall back to support_condition if available
            return torch.all(self.data.support_condition, dim=1, keepdim=True)
        else:
            raise AttributeError(f"'{self.__class__.__name__}' object has no attribute 'is_support' or 'support_condition'")
    ...
    def add_node(self, name: str, **kwargs):
        ...
        if unexpected_attrs:
            raise ValueError(f"Unexpected node attributes: {unexpected_attrs}. Expected: {list(self.node_attr_list)}")
        ...
        for attr in self.node_attr_list:
            if attr in kwargs:
                value = kwargs[attr]
                if not isinstance(value, torch.Tensor):  # Cast non-tensor attributes to tensor
            ...
            elif attr in self.default_attrs:  # If attribute is not provided, set it to default value
                value = self.default_attrs[attr]
            else:  # If attribute has no default value, set it to zero
            ...
            setattr(self.data, attr, torch.cat([getattr(self.data, attr), value.unsqueeze(0)], dim=0))
    ...
    def add_edge(self, src: str, dst: str, **kwargs):
        ...
        if unexpected_attrs:
            raise ValueError(f"Unexpected edge attributes: {unexpected_attrs}. Expected: {list(self.edge_attr_list)}")
        ...
        self.data.directed_mask = torch.cat([self.directed_mask, torch.tensor([[True], [False]])], dim=0)
        self.data.reciprocal_edge = torch.cat([self.reciprocal_edge, torch.tensor([[self.num_edges + 1], [self.num_edges]])], dim=0)

        # Add edge to edge_index
        src_index, dst_index = self.node_name_to_index[src], self.node_name_to_index[dst]
        new_edge = torch.tensor([[src_index, dst_index],
                                 [dst_index, src_index]])
        ...
        for attr in self.edge_attr_list:
            ...
            if value.dim() == 0:
                value = value.unsqueeze(0)
            setattr(self.data, attr, torch.cat([getattr(self.data, attr), value.unsqueeze(0), value.unsqueeze(0)], dim=0))
    ...
    def verify_equilibrium(self, tolerance=1e-7, verbose=False, **kwargs):
        ...
        if 'is_support' not in kwargs:
            if 'is_support' in self.node_attr_list:
                ...
                kwargs['is_support'] = torch.all(self.support_condition, dim=1, keepdim=True)
    ...
    def mpcem(self, **kwargs):
        edge_mask = ~(self.is_trail_edge.view(-1) & ~self.directed_mask.view(-1))  # Create semi-directed graph
    ...
    def edge_attr_to_undirected(self, edge_attr, mask):
        ...
        value[mask] = edge_attr  # set defined values
        value[~mask] = value[self.reciprocal_edge[~mask].view(-1)]  # set missing reciprocal values
        return value
        ...
        for arg in pos_args:
            if arg not in kwargs:
                ...
                if edge_mask is not None:
                    ...
                    elif arg == 'edge_index':  # TODO: Make more general for higher-dimensional attributes
                        value = value[:, edge_mask]
