        ...
        for key in sorted(keys):
            ...
            def classify(v):
                ...
                if isinstance(v, (list, tuple, np.ndarray)):
                    L = len(v)
                    if 1 <= L <= 4 and all(isinstance(x, (int, float, np.integer, np.floating, bool)) for x in v):
                        return ("numvec", L)
                    return ("str", 1)
                if v is None:
                    return ("unknown", 1)
            ...
            arities = [classify(v)[1] for v in vals if v is not None and classify(v)[0] != "str"]
