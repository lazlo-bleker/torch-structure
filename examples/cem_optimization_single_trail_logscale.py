...
def obj_func(trail_lengths_logscale, struc_data : StructData):
    trail_lengths_logscale = (
        trail_lengths_logscale.float()
    )  # Cast to 32-bit float (ToDo: add easy 64-bit support)

    # Update the force vector with optimization variables
    trail_lengths = torch.exp(trail_lengths_logscale)
...
initial_values_logscale = torch.log(initial_values)

# Run optimization using scipy
result = minimize(
    fun=obj_func,
    args=(trail),
    x0=initial_values_logscale.detach().numpy(),
    method="SLSQP",
    jac=True,
