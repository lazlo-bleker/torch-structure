...
if DARK:
    ...
    # with set_axis_off() (default facecolor (0.95,0.95,0.95,0.5)), and
    # Axes3D also carries its own inherited opaque 2D-style background
    # patch independent of those panes -- both need to be made transparent
    # too, or they show up as a big solid rectangle once composited onto
...
# size the bounding box to the actual per-axis data extent (the tonne's z
# range is much smaller than its x/y range, so a forced cubic box wastes
# a lot of vertical headroom) instead of the default equal_axes cube.
u = torch.linspace(0, 1, num_rows).repeat_interleave(num_cols).unsqueeze(1)
v = torch.linspace(0, 1, num_cols).repeat(num_rows).unsqueeze(1)
coords = f(x_unit_coord=u, y_unit_coord=v)
pad = 0.05
...
ax.set_ylim3d(y_min - pad, y_max + pad)
ax.set_zlim3d(z_min - pad, z_max + pad)
ax.set_box_aspect((x_range + 2 * pad, y_range + 2 * pad, z_range + 2 * pad))
ax.set_proj_type("ortho")

path = "docs/assets/grid_axonometric_dark.png" if DARK else "docs/assets/grid_axonometric.png"
...
if DARK:
    ...
    # the grid's silhouette isn't symmetric within its own bounding box (the
    # isometric skew makes one corner reach further than the other), so pad
    ...
    shift = int(-0.02 * W2)
    final = Image.new("RGBA", (W2 + 2 * margin_x, H2 + margin_top + margin_bottom), (0, 0, 0, 0))
    final.paste(new_img, (margin_x + shift, margin_top))
    ...
    # top whitespace, similarly to the chain figure.)
    arr = np.array(img)
    ...
    margin_bottom = int(0.06 * max(W2, H2))
    shift = int(-0.02 * W2)
    final = Image.new("RGB", (W2 + 2 * margin_x, H2 + margin_top + margin_bottom), (255, 255, 255))
    final.paste(new_img, (margin_x + shift, margin_top))
