...
nodes  = [(u, v) for u in range(n) for v in range(m)]
h_edges = [(u, v) for u in range(n) for v in range(m - 1)]
v_edges = [(u, v) for u in range(n - 1) for v in range(m)]
...
# ── Panel 1: Node defaults ── (title placed after layout to align with add_chain)
_node_title_text = "Node defaults"

for u, v in h_edges:
    ax1.plot([px(u, v), px(u, v+1)], [py(u, v), py(u, v+1)], color=DGREY, lw=1.5)
for u, v in v_edges:
    ax1.plot([px(u, v), px(u+1, v)], [py(u, v), py(u+1, v)], color=DGREY, lw=1.5)

for u, v in nodes:
    ax1.scatter(px(u, v), py(u, v), color=NODE_FILL, s=80, zorder=3,
                edgecolors=ORANGE, linewidths=1.2)
    ax1.text(px(u, v) + D, py(u, v) + D,
             f"({u}, {v})", ha="left", va="bottom", fontsize=12.5,
             color=TEXT)
    ax1.text(px(u, v) - D, py(u, v) - D,
             f"({xu(u):.2f}, {yv(v):.2f})", ha="right", va="top", fontsize=12.5,
             color=DGREY)
...
for u, v in nodes:
    ax2.scatter(px(u, v), py(u, v), color=NODE_FILL, s=80, zorder=3,
...
# Single, uniform rule for every edge (both directions): the label pair sits
# at the edge's own midpoint, offset along the edge's normal, and rotated to
# read along the edge's own direction. Walking from source to destination,
# the index label goes to the left, the coord label to the right.
...
# dir=0: (u,v)→(u,v+1) — goes +y (vertical in plot)
for u, v in h_edges:
    label_c = f"({xu(u):.2f}, {(yv(v)+yv(v+1))/2:.2f})"
    plot_edge_with_normal_labels(ax2, px(u, v), py(u, v), px(u, v+1), py(u, v+1),
                                  f"({u}, {v})", label_c, "-")

# dir=1: (u,v)→(u+1,v) — goes +x (horizontal in plot)
for u, v in v_edges:
    label_c = f"({(xu(u)+xu(u+1))/2:.2f}, {yv(v):.2f})"
    plot_edge_with_normal_labels(ax2, px(u, v), py(u, v), px(u+1, v), py(u+1, v),
                                  f"({u}, {v})", label_c, "--")

leg = [
    mpatches.Patch(color=TEXT,        label="(u_ind, v_ind)"),
    mpatches.Patch(color=SWATCH_GREY, label="(x_unit_coord, y_unit_coord)"),
...
# Align arrow start and "Node defaults" title with add_chain's left edge.
CHAIN_AXES_LEFT_FRAC = 0.010714  # measured from gen_chain_defaults_fig.py
CHAIN_ARROW_FRAC     = 0.055195  # measured from gen_chain_defaults_fig.py
fig.canvas.draw()

# Place "Node defaults" title at the same figure-x as add_chain's axes left edge.
...
_title_x_axes = (_target_px - _ax1_left_px) / _ax1_width_px  # axes fraction (negative = left of axes)
_title_y_axes = 1.0 + 4 / (ax1.get_position().height * fig.get_figheight() * 72)  # 4pt above top
ax1.text(_title_x_axes, _title_y_axes, _node_title_text,
         fontsize=16, va="bottom", transform=ax1.transAxes, clip_on=False)
...
ax1.text(ox + 0.15, oy, "u_ind", ha="left", va="center", fontsize=12.5, color=TEXT)
ax1.annotate("", xy=(ox, oy + 0.15), xytext=(ox, oy),
             arrowprops=dict(arrowstyle="->", color=TEXT, lw=1.2))
ax1.text(ox + 0.01, oy + 0.15, "v_ind", ha="left", va="center", fontsize=12.5, color=TEXT)

OUT_PATH = "docs/assets/add_grid_defaults_dark.png" if DARK else "docs/assets/add_grid_defaults.png"
